...
bool ParseCommandLine(const std::vector<std::string>& args,
  ...
  for (; cur_idx < args.size(); ++cur_idx) {
    ...
    } else if (cur_arg == "--printer") {
      if (options->output_format != OUTPUT_NONE) {
        fprintf(stderr, "Duplicate or conflicting --printer argument\n");
        return false;
      }
      options->output_format = OUTPUT_PRINTER;
#endif  // _WIN32
...
bool RenderPage(const std::string& name,
  ...
  if (bitmap) {
    ...
    switch (options.output_format) {
      ...
      case OUTPUT_PRINTER:
        image_file_name =
            WritePrinter(name.c_str(), page_index, buffer, stride, width, height);
        break;
